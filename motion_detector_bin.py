...
def motion_detection(video_array,x1=0,x2=-1,z1=0,z2=-1):
    ...
    for i in range(1,NIMG):   #Empezamos en 1 porque el frame 0 lo tomamos de referencia
        ...
        if np.sum(frame_gray) in (255*frame_gray.shape[0]*frame_gray.shape[1], 0):
            ...
            if st is not None:
                ...
                good_old = p0[st == 1] 
                good_new = p1[st == 1] 
                [dx,dz] = np.mean((good_new - good_old),axis=0)  # Calcula el desplazamiento medio de todos los puntos elegidos
    ...
    figure(2).clf()
    plot(dxvf,label='Velocity');    
    xlabel('frame',fontsize=14)
    ylabel('pixel/frame',fontsize=14)
    legend(fontsize=14);
