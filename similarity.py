...
from video_player import normalize
...
def init_similarity(encoder_filename="convAE_encoder_v2.h5"):
    ...
    encoder_filepath = os.path.join(encoder_directory, encoder_filename)
    encoder = tf.keras.models.load_model(encoder_filepath)

    # Load image database
    database_display = np.load(os.path.join(encoder_directory, "database_display.npy"))
    ...
    database_encoded = np.load(os.path.join(encoder_directory, "database_encoded.npy"))
    ...
    database_filenames = np.load(os.path.join(encoder_directory, "database_filenames.npy"))
