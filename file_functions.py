...
class BinFile:
    ...
    def __init__(self, filepath):
        ...
        self.firmware_boot_version = ReadBINFileField_STRING(f,e)
...
class Dataset:
    ...
    def __init__(self, binFile: BinFile, med_kernel = 3, resample = 1000, new_db_range = 0, n_time_filter = 3):
        ...
        self.n_time_filter = n_time_filter # Tamaño del filtro de media móvil en el tiempo
                                           # para desacrivar = 0
        ...
        # Si está activado el filtro de media móvil en el tiempo lo aplico
        if self.n_time_filter > 0:
            tmp_buffer = self.bscan.copy()
            for i in range(self.n_time_filter,self.nimg):
                self.bscan[:,:,i] = np.mean(tmp_buffer[:, :, i - self.n_time_filter:i], 2)
            del tmp_buffer
    ...
    def ScanConvert(self, InterpType = 'bilinear'):
        ...
        if InterpType == 'nearest':
            ...
            self.frames = np.ones((self.frame_ny, self.frame_nx, self.nimg), dtype='h') * (-self.dB_range)
...
def ReadBINFileField_STRING(file, e):
